...
- [ ] **TD-15 — Catalog Value Objects.** Revisit when a field gains meaningful invariants or behavior: introduce `Money` when price needs currency or monetary operations, and `ProductSlug`/`CategorySlug` when slug rules outgrow `SlugUtils`. Do not wrap primitive fields without a clear domain benefit.
- [ ] **TD-16 — User and Address Value Object review.** Start Day 16 with `Email` because it needs normalization, format validation, and value-based equality. Keep `UserId` and `AddressId` as `UUID` until the project adopts typed IDs consistently across modules; introduce `FullName` only when name rules become explicit, and `PhoneNumber` only after selecting a country/international format. Keep `Address` as an Entity because it has identity, ownership, mutable state, and a lifecycle.
